...
Cajero.prototype = {
    ...
    guardarCobro: function()
    {
        new Ajax.Request(this.urlGuardar, {
            method: 'post',
            params: {
                'data[Pago][mesa_id]': this.mesa_id,
                'data[Pago][tipo_de_pago_id]': this.pagos[0],
                'data[Pago][valor]': this.total
            },
    ...
    guardarCobroDeUna: function(pago_id)
    {
        ...
        new Ajax.Request(this.urlGuardar, {
            ...
            parameters: {
                'data[Pago][mesa_id]': this.mesa_id,
                'data[Pago][tipo_de_pago_id]': pago_id,
                'data[Pago][valor]': this.total
            },
            onSuccess: function() {
                //$$("#mesa-id-"+this.mesa_id).hide();
...
//console.debug($('mesa-id-'+m.Mesa.id));
        if ($('mesa-id-'+m.Mesa.id) != null) {
            $('mesa-id-'+m.Mesa.id).remove();
        }

        var elemento = new Element('li',{
            'id':'mesa-id-'+m.Mesa.id,
