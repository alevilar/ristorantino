   ...
   */
    ...
    enviarComanda: function(imprimir){
        ...
        var formulario = new Element('form', {
            'name':'Comanda',
            'action':this.urlEnviarComanda+'/mesa_id:'+this.mesa.id+'/mozo_id:'+this.mozo.id
            });
		 	
        //voy armando el formulario y generando el $this->data[]
        formulario.appendChild(new Element('input', {
            'name': 'data[imprimir]'
        }).setValue(imprimir)); //dice si hay que mandar a imprimr o solo guardar los datos
		  
		  
        formulario.appendChild(new Element('input', {
            'name': 'data[Comanda][prioridad]'
        }).setValue(this.prioridad)); //avisa si la comanda va con prioridad o no
        formulario.appendChild(new Element('input', {
            'name': 'data[Comanda][mesa_id]'
        }).setValue(this.mesa.id));
        formulario.appendChild(new Element('input', {
            'name': 'data[Comanda][observacion]'
        }).setValue($F('comanda-observacion')));
        // le agrego los productos con su rspectiva catidad
        var count = 0;
        this.productos.each(function(p){
            if(p.cantidad > 0){
                formulario.appendChild(new Element('input', {
                    'name': 'data['+count+'][DetalleComanda][producto_id]'
                    }).setValue(p.getId()));
                formulario.appendChild(new Element('input', {
                    'name': 'data['+count+'][DetalleComanda][cant]'
                    }).setValue(p.getCantidad()));
                ...
                formulario.appendChild(new Element('input', {
                    'name': 'data['+count+'][DetalleComanda][es_entrada]'
                    }).setValue(prod_entrada));
                formulario.appendChild(new Element('input', {
                    'name': 'data['+count+'][DetalleComanda][comandera_id]'
                    }).setValue(p.getComanderaId()));
                ...
                p.saboresSeleccionados.each(function(sabor){
                    ...
                    formulario.appendChild(new Element('input', {
                        'name': 'data['+count+'][DetalleSabor]['+cont2+'][sabor_id]'
                        }).setValue(sabor.id));
                    cont2++;
                });
    ...
    seleccionarSabores:function(producto)
    {
        ...
        this.productoCreandoSabores.sabores.each(function(sabor){
            var li = new Element('li');
            var a = new Element('a',{
                'href':'#seleccionarSabor',
                'class':'boton-ancho-largo'
            });
            a.update(sabor.name);
            a.observe('click', a.setStyle.curry({
